   ...
   for day in $days; do
      if [[ -d $inpath ]];then
        ...
        for hh in $hours; do
           ...
           #inputfiles=$(python run_viirs2ioda_n20.py ${year} ${month} ${day} ${hh} ${inpath} ${outpath})
           #echo $inputfiles

           # the processed output filename
           #outputfile=$outpath/${year}${month}${day}/"ioda.viirs_n20."$year$month$day$hh".nc4"
           #outputfile=$outpath/${year}${month}${day}"/gdas.t"$hh"z.viirs_n20."$year$month$day$hh".nc4"
           #python Edit_yaml_weight.py $year$month$day$hh $outputfile $thinning "4" "n20" $inputfiles
