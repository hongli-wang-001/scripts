...
def update_grid_with_observations_2b(lats_obs, lons_obs, values_obs, lats_grid, lons_grid, grid_size=0.1):
    # Convert lons_obs from [-180, 180] to [0, 360]
    lons_obs = np.where(lons_obs < 0, lons_obs + 360, lons_obs)

    interpolated_values = np.full(lats_grid.shape, np.nan)
    dis_values = np.full(lats_grid.shape, np.nan)
    dis_values[:] = 13.0
    for obs_idx in range(len(lats_obs)):
        lat_obs = lats_obs[obs_idx]
        lon_obs = lons_obs[obs_idx]
        ...
        found_valid_point = False
        dis_temp = 100.0
        ni, nj = -1, -1  # Initialize as invalid
        for i, j in zip(lat_indices, lon_indices):
                distance = great_circle_distance(lat_obs, lon_obs, lats_grid[i, j], lons_grid[i, j])
                #print(f'Distance to Grid Point (Lat: {lats_grid[i, j]}, Lon: {lons_grid[i, j]}): {distance:.2f} km')
                if distance <= dis_temp:
                   ...
                   ni , nj = i , j
                   print(f'Observation at (Lat: {lat_obs}, Lon: {lon_obs}, Dis: {dis_temp}')
        if ni != -1 and nj != -1 and dis_temp <= dis_values [ni,nj]:
                    print(f'Observation at (Lat: {lat_obs}, Lon: {lon_obs}, Value: {value_obs}) updates Grid Point at (Lat: {lats_grid[i, j]}, Lon: {lons_grid[i, j]})')
                    dis_values [ni,nj] = dis_temp 
                    if 0 <= ni < lats_grid.shape[0] and 0 <= nj < lons_grid.shape[1]:
                                if np.isnan(interpolated_values[ni, nj]):
                                    interpolated_values[ni, nj] = value_obs
                                #else:
...
def write_to_netcdf_pm25(lats_grid, lons_grid, interpolated_values, output_file):
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds:
        ...
        interp_var = ds.createVariable('pm25', 'f4', ('y', 'x'), zlib=True, complevel=4)
        ...
        lon_var.units = 'degrees_east'
        interp_var.units = 'µg/m³'
        interp_var.long_name = 'PM2.5 concentration'
...
def write_to_netcdf(lats_grid, lons_grid, interpolated_values, output_file,
    ...
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds:
        ...
        lat_var = ds.createVariable('latitude', 'f4', ('y', 'x'))
        lon_var = ds.createVariable('longitude', 'f4', ('y', 'x'))
        interp_var = ds.createVariable(var_name, 'f4', ('y', 'x'), zlib=True, complevel=4)
        ...
        lat_var.units = 'degrees_north'
        lon_var.units = 'degrees_east'
        interp_var.units = var_unit
        interp_var.long_name = 'Interpolated observation field'

        ds.title = f'{var_name} Interpolation Output'
        ds.projection = 'Lambert Conformal Conic'
...
def main():
    ...
    obs_var_path = config['obs_variable']
    grid_file = config['grid_file']
    output_file = config['output_file']
    plot_file = config['plot_file']
    obs_nc_var = config.get('obs_variable_netcdf')
    obs_unit = config.get('obs_variable_unit_netcdf')

    # Load observation data
    f1 = nc.Dataset(input_file, 'r')
    lons_obs = f1['MetaData/longitude'][:]
    lats_obs = f1['MetaData/latitude'][:]
    values_obs = f1[obs_var_path][:]

    if values_obs.size > 0:
        # Load model grid
        ...
        print("Obs Values:")
        print(f"Min: {np.nanmin(values_obs)}, Max: {np.nanmax(values_obs)}, "
              f"Mean: {np.nanmean(values_obs)}, Std: {np.nanstd(values_obs)}")

        interpolated_values = update_grid_with_observations_2b(lats_obs, lons_obs, values_obs, lats_grid, lons_grid)

        print("Interpolated Values:")
        print(f"Min: {np.nanmin(interpolated_values)}, Max: {np.nanmax(interpolated_values)}, "
              f"Mean: {np.nanmean(interpolated_values)}, Std: {np.nanstd(interpolated_values)}")

        print(f'Valid interpolated points: {np.count_nonzero(~np.isnan(interpolated_values))}, '
              f'valid obs: {np.count_nonzero(~np.isnan(values_obs))}')

        plot_surface(lats_grid, lons_grid, interpolated_values, f'{obs_nc_var}', plot_file)
        write_to_netcdf(lats_grid, lons_grid, interpolated_values, output_file,
                        var_name=obs_nc_var, var_unit=obs_unit)
    else:
        print("No observation data found. Skipping interpolation and output.")
