...
min_lengthscale = 2  # Minimum lengthscale (units in grid cells)
max_lengthscale = x_dim / 2  # Maximum lengthscale (half of grid size)
...
def truncate_dct(dct_data, min_lengthscale, max_lengthscale):
    """ Truncate DCT coefficients based on specified min and max lengthscales """
    nrows, ncols = dct_data.shape
    freq_limit_min = 1 / max_lengthscale
    freq_limit_max = 1 / min_lengthscale
    
    # Frequency grid creation
...
def calculate_wavelengths(u_freq, v_freq, min_lengthscale, max_lengthscale):
    ...
    # Apply min and max lengthscale constraints
    valid_wavelengths = wavelengths[np.isfinite(wavelengths)]
    
    min_wavelength = min_lengthscale
    max_wavelength = max_lengthscale
...
for i, z_level in enumerate(z_levels):
    ...
    # Apply truncation based on new lengthscale constraints
    dct_data_truncated, u_freq, v_freq = truncate_dct(dct_data, min_lengthscale, max_lengthscale)
    ...
    min_wavelength, max_wavelength = calculate_wavelengths(u_freq, v_freq, min_lengthscale, max_lengthscale)

    print(f"\nFor z={z_level} with min lengthscale {min_lengthscale} and max lengthscale {max_lengthscale}:")
