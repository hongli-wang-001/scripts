...
for idx in scales_to_plot:
    ...
    plt.imshow(apply_idct_2d(dct_data), cmap='gray')
    plt.colorbar()
    ...
    for trunc in truncations:
        ...
        plt.colorbar()
        plt.text(0.1, 0.9, f'Wavelength X: {avg_wavelength_x:.2f}', fontsize=10, color='red', bbox=dict(facecolor='white', alpha=0.7))
        plt.text(0.1, 0.6, f'Wavelength Y: {avg_wavelength_y:.2f}', fontsize=10, color='red', bbox=dict(facecolor='white', alpha=0.7))

# Save the figure
plt.tight_layout()
plt.savefig('dct_truncation_reconstruction_with_scales.png')
print("DCT truncation reconstruction with scales figure saved as 'dct_truncation_reconstruction_with_scales.png'.")
