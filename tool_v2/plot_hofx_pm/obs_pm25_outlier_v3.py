...
def print_stats(data, label):
    print(f"{label} - Mean: {np.mean(data):.2f}, Median: {np.median(data):.2f}, Std Dev: {np.std(data):.2f}")

# Total number of observations
total_obs = pm_flat.size

# Number of outliers
num_outliers = np.sum(outlier_mask)
percentage_outliers = (num_outliers / total_obs) * 100

# Print statistics with and without outliers
print_stats(pm_flat, "With outliers")
print_stats(pm_flat[~outlier_mask], "Without outliers")
...
# Plot PM2.5 data
sc = plt.scatter(tlon, tlat, s=2, c=pm, linewidth=.50, cmap='jet', norm=colors.BoundaryNorm(boundaries=myvalues, ncolors=256))
