...
#    print(f"{label} - Mean: {np.mean(data):.2f}, Median: {np.median(data):.2f}, Std Dev: {np.std(data):.2f}, RMS: {np.sqrt(np.mean(data**2)):.2f}")
    print(f"{label}  - Min: {np.min(data):.2f},  - Max: {np.max(data):.2f},- Mean: {np.mean(data):.2f}, Median: {np.median(data):.2f}, Std Dev: {np.std(data):.2f}, RMS: {np.sqrt(np.mean(data**2)):.2f}")

#myvalues=[2,3, 5, 7, 10, 15, 20, 30, 50, 70, 100]
#myvalues=[-20, -10, -5, -3, -1, 1, 3, 5, 10, 20]
myvalues=[-20, -10, -5, -3, -1, -0.6, -0.3, -0.1,0.1, 0.3, 0.6, 1, 3, 5, 10, 20]

f1=nc.Dataset(sys.argv[1],'r')
tlon=f1['MetaData/longitude'][:]
tlat=f1['MetaData/latitude'][:]
pma=f1['ObsValue/particulatematter2p5Surface'][:]-f1['hofx1/particulatematter2p5Surface'][:]
pmb=f1['ObsValue/particulatematter2p5Surface'][:]-f1['hofx0/particulatematter2p5Surface'][:]
product = pma * pmb
# Calculate the average of the product
average_product = np.mean(product)
median_product = np.median(product)
# Calculate the sum of the product
sum_of_product = np.sum(product)
print("product:", sum_of_product, average_product, median_product)
pma_p=pma-np.mean(pma)
pmb_p=pmb-np.mean(pmb)
product = pma_p * pmb_p
# Calculate the average of the product
average_product = np.mean(product)
# Calculate the sum of the product
sum_of_product = np.sum(product)
print("product:", sum_of_product, average_product,np.median(product))

pm=np.abs(pma)-np.abs(pmb)
print_stats(pm, "OmA-OmB")
