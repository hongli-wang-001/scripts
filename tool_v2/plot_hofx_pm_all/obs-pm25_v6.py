#!/usr/bin/env python
# Plot PM2.5 with custom colormap and colorbar handling
...
def create_custom_cmap():
    ...
    cmap = mcolors.ListedColormap([
        (0.8627, 0.8627, 1.0000),  # Color for values below 3
        ...
        (0.7843, 0.7843, 0.7843)  # Color for values above 750
    ])
    cmap.set_under((0.8627, 0.8627, 1.0000))  # Color for values below 3
    cmap.set_over((0.9412, 0.9412, 0.9412))   # Color for values above 750
    return cmap
...
# Define boundaries and create the colormap and norm
myvalues = [0.,3., 6., 9., 12., 15., 35., 55., 75., 100., 125., 150., 250., 300., 400., 500., 600., 750.]
cmap = create_custom_cmap()
norm = mcolors.BoundaryNorm(boundaries=myvalues, ncolors=cmap.N)

# Create the plot
fig, ax = plt.subplots(figsize=(12, 8), subplot_kw={'projection': ccrs.PlateCarree()})

# Scatter plot
scatter = ax.scatter(tlon, tlat, s=7, c=pm, linewidth=0.50, cmap=cmap, edgecolor='none', norm=norm)
...
# Add colorbar with extended ranges
...
# Add ticks to show extended ranges
#cbar_ticks = [3, 6, 12, 35, 75, 100, 150, 250, 500, 750]
#cbar.set_ticks(cbar_ticks)
#cbar.set_ticklabels([f'< {cbar_ticks[1]}'] + [str(tick) for tick in cbar_ticks[1:]])

# Add text with data statistics
stats_text = (f'Min: {min_pm:.2f}\nMax: {max_pm:.2f}\nMean: {mean_pm:.2f}\n'
              f'Median: {median_pm:.2f}\nStd Dev: {std_pm:.2f}')
plt.text(0.05, 0.35, stats_text, transform=ax.transAxes, fontsize=12,
