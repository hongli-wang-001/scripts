...
def add_data_2(infile):
    ...
    # Update df['time'] with the UTC datetime
    df['time'] = df['datetime_utc']

    # Calculate local time based on datetime_local
    df['time_local'] = df['datetime_local']

    # Update the utcoffset based on the difference between local and UTC time
    df['utcoffset'] = (df['datetime_local'] - df['datetime_utc']).dt.total_seconds() / 3600  # Convert seconds to hours

    # Use sensor_index to create siteid, ensuring it's a string and zero-padded if needed
    df['siteid'] = df['sensor_index'].astype(str).str.zfill(9)

    # Set stationElevation to 0 and height to 10
    df['elevation'] = 0  # Fill with 0
    ...
    float_cols = ['humidity_a', 'temperature_a', 'pressure_a', 
                  ...
                  'pm10.0_cf_1_a', 'pm10.0_cf_1_b', 'pm25', 'ozone']
    
    for col in float_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')
#pm25_pa, 0.5081274287627826
#humidity_a, -0.04488485501024457
#intercept, 4.893075882425148
#    df['PM2.5'] = df['pm25']
    df['PM2.5'] = df['pm25']*0.5081274287627826-0.04488485501024457*df['humidity_a']+4.893075882425148
    df['OZONE'] = df['ozone']
    df['pressure'] = df['pressure_a']

    # Drop duplicates if necessary
    df.drop_duplicates(inplace=True)
    # Remove the 'pressure_a' column
    df.drop(columns=['pressure_a'], inplace=True)
    df = filter_bad_values(df)
    # Apply superob function
    superob_df = superob(df)
...
def superob(df):
    ...
    # Create bins for latitude and longitude
    #df['lat_bin'] = (df['latitude'] * 20).round() / 20
    #df['lon_bin'] = (df['longitude'] * 20).round() / 20

    # Group by binned latitude and longitude
    superob_df = df.groupby(['lat_bin', 'lon_bin'], as_index=False).agg(
        longitude=('longitude', 'mean'),
        latitude=('latitude', 'mean'),
        PM2_5_mean=('PM2.5', 'mean'),  # Temporary mean column for PM2.5
        OZONE_mean=('OZONE', 'mean'),
        humidity_a_mean=('humidity_a', 'mean'),
        temperature_a_mean=('temperature_a', 'mean'),
        pressure_mean=('pressure', 'mean'),
        elevation_mean=('elevation', 'mean'),
        obs_count=('PM2.5', 'size'),  # Count of observations
        PM2_5_std=('PM2.5', lambda x: x.std() if len(x) > 1 else np.nan),  # Std dev if more than 1 observation
        OZONE_std=('OZONE', lambda x: x.std() if len(x) > 1 else np.nan),  # Std dev if more than 1 observation
    )

    # Find the nearest original entries for non-aggregated fields
    nearest = df.groupby(['lat_bin', 'lon_bin']).agg({
        'siteid': 'first',
        'time_stamp': 'first',
    ...
    superob_df = superob_df.merge(nearest, on=['lat_bin', 'lon_bin'])

    # Rename columns back to original names
    superob_df.rename(columns={
        'PM2_5_mean': 'PM2.5',
        'OZONE_mean': 'OZONE',
        'humidity_a_mean': 'humidity_a',
        'temperature_a_mean': 'temperature_a',
        'pressure_mean': 'pressure',
        'elevation_mean': 'elevation'
    }, inplace=True)

    # Drop the temporary binned columns
    superob_df.drop(columns=['lat_bin', 'lon_bin'], inplace=True)

    return superob_df

def superob2(df, center_latitude):
    # Convert 13 km to degrees (approximately)
    km_per_degree_lat = 111  # latitude
    km_per_degree_lon = 111 * np.cos(np.radians(center_latitude))  # longitude

    # Calculate the number of degrees for 13 km
    degree_lat = 13 / km_per_degree_lat
    degree_lon = 13 / km_per_degree_lon

    # Create bins for latitude and longitude
    df['lat_bin'] = (df['latitude'] / degree_lat).round() * degree_lat
    df['lon_bin'] = (df['longitude'] / degree_lon).round() * degree_lon
    ...
    superob_df = df.groupby(['lat_bin', 'lon_bin'], as_index=False).agg(
        ...
        PM2_5=('PM2.5', 'mean'),
        OZONE=('OZONE', 'mean'),
        humidity_a=('humidity_a', 'mean'),
        temperature_a=('temperature_a', 'mean'),
        pressure=('pressure', 'mean'),
        elevation=('elevation', 'mean'),
        obs_count=('PM2.5', 'count'),
        PM2_5_std=('PM2.5', 'std')
    )
...
if __name__ == '__main__':
    ...
    #print('model_hgt:',hgt)
    #print('interpolated_hgt:',interpolated_hgt)
    # Count missing values
    missing_values_count = np.isnan(interpolated_hgt).sum()
    print(f'Number of missing values: {missing_values_count}')
    # Print min and max values
    min_value = np.nanmin(interpolated_hgt)
    max_value = np.nanmax(interpolated_hgt)
    print(f'Minimum value of interpolated_hgt: {min_value}')
    print(f'Maximum value of interpolated_hgt: {max_value}')
    ...
    data['height'] = interpolated_hgt + 10.0
    data['pressure'] = np.array(f3['pressure'])
...
#        ioda_data[(variable, obsErrName)] = np.full(nlocs, 0.05*data[variable], dtype=np.float32)
        ioda_data[(variable, obsErrName)] = np.full(nlocs, 0.15*data[variable], dtype=np.float32)
        print('Datatype:', data[variable].dtype)
        ioda_data[(variable, obsErrName)] = np.where(0.15*data[variable]>0.15,0.15*np.float32(data[variable]),0.15)
#        ioda_data[(variable, qcName)] = np.full(nlocs, 2, dtype=np.int32)
        # Check if data[variable] is non-missing
        #if np.any(~np.isnan(data[variable])):  # Check for non-missing values
        if np.any(~np.isnan(data['height'])):  # Check for non-missing values
           ioda_data[(variable, qcName)] = np.full(nlocs, 2, dtype=np.int32)
        else:
           ioda_data[(variable, qcName)] = np.full(nlocs, -2, dtype=np.int32)
