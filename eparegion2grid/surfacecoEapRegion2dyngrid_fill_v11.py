...
def write_to_netcdf(lats_grid, lons_grid, interpolated_values, output_file):
    with nc.Dataset(output_file, 'w', format='NETCDF4') as ds:
        ...
        interp_var = ds.createVariable('epa_region', 'f4', ('y', 'x'), zlib=True, complevel=4)
        ...
        lon_var.units = 'degrees_east'
        interp_var.units = ' '
        interp_var.long_name = 'epa_region'

        # Optionally: Add global attributes
        ds.title = ' EPA REGION Interpolation Output'
        ds.projection = 'Lambert Conformal Conic'

def convert_region_to_int_1(val):
    match = re.fullmatch(r'R(\d+)', val)
    if match:
        return int(match.group(1))
    return np.nan

def convert_region_to_int(val):
    if isinstance(val, bytes):
        val = val.decode('utf-8')  # Convert bytes to string
    match = re.fullmatch(r'R(\d+)', val)
    if match:
        return int(match.group(1))
    return np.nan
...
def main():
    ...
    #values_obs = f1['CO'][:].flatten()  # assuming 'BC' is 2D or 1xN
    cvalues_epa = f1['epa_region'][:].flatten()

    # Convert all values
    values_obs = np.array([convert_region_to_int(val) for val in cvalues_epa])
