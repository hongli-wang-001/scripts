...
file_pattern = 'con/aqm.202009*/00/aqm.t00z.phy.f001.nc'
...
with Dataset(first_file, 'r') as nc:
    # Get dimensions and shape from hpbl 
    pm25_tot_dims = nc.variables['hpbl'].dimensions
...
# Define file pattern and exclude pattern
file_pattern2 = 'pmf/aqm.202009*/00/aqm.t00z.phy.f001.nc'
...
with Dataset(first_file2, 'r') as nc2:
    ...
    pm25_tot_dims2 = nc.variables['hpbl'].dimensions
...
for file in file_list:
    with Dataset(file, 'r') as nc:
        ...
        if 'hpbl' in nc.variables:
            data_pm25_tot.append(nc.variables['hpbl'][:])
            #data_pm25_ave.append(nc2.variables['pm25_ave'][:])
            print(f"Find hpbl in file {file}")
        else:
            print(f"Missing hpbl in file {file}")

for file in file_list2:
    with Dataset(file, 'r') as nc2:
        if 'hpbl' in nc2.variables:
            #data_pm25_tot.append(nc.variables['pm25_ave'][:])
            data_pm25_ave.append(nc2.variables['hpbl'][:])
            print(f"Find hpbl in file {file}")
        else:
            print(f"Missing hpbl in file {file}")
