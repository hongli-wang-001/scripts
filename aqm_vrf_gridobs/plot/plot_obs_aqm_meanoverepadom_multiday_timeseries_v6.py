...
# Load the epa_region mask from eparegion.nc
mask_ds = xr.open_dataset('eparegion.nc')
epa_region_mask = mask_ds['epa_region'].values
mask_ds.close()
    ...
    for file_path in file_list:
        try:
            ...
            if var_obs_name in ds and var_fcst_name in ds:
                # Apply the epa_region mask: only consider values where epa_region is 8, 9, or 10
                mask = np.isin(epa_region_mask, [8, 9, 10])

                # Mask the pm25 and pm25_diff variables
                pm25_data = ds[var_obs_name].values
                pm25_diff_data = ds[var_fcst_name].values

                # Apply the mask
                pm25_masked = np.ma.masked_where(~mask, pm25_data)
                pm25_diff_masked = np.ma.masked_where(~mask, pm25_diff_data)

                # Calculate means, skipping NaN values
                pm25_mean = np.nanmean(pm25_masked)
                pm25_diff_mean = np.nanmean(pm25_diff_masked)
...
csv_filename = f"pm25_meanoverepadom_obs_aqm_multiday_i{init_utc}.csv"
...
#axes.set_title("Time Series of PM$_{2.5}$ Mean")
#axes.set_ylabel('PM$_{2.5}$ (µg/m³)')
axes.set_title("Time Series of PM₂.₅ Mean")
axes.set_ylabel('PM₂.₅ (µg/m³)')
...
plt.tight_layout()
plt.savefig(f"pm25_meanoverepadom_obs_aqm_multiday_i{init_utc}.png", dpi=300)
plt.show()
