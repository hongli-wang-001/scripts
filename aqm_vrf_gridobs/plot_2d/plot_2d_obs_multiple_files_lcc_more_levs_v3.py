...
#    )
    ...
    cb = plt.colorbar(scatter, ax=ax, orientation='vertical', pad=0.02, shrink=0.85)
    cb.set_label('PM2.5 (µg/m³)')
    cb.set_ticks(aqi_levels)

    plt.title(f'PM2.5: Observations @ {title_time}')
    plt.tight_layout()
    plt.savefig(f"obs_pm25_{time_str}.png", dpi=300)
    plt.close()
