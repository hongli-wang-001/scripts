...
aod_values = data[:, 3]  # 3th column obs
bias_values =  data[:, 4] # xf-obs
aod_values[aod_values == -99] = np.nan
bias_values[bias_values == -99] = np.nan

# Retrieve cmaq aod s = obs + (xf-obs) 
aod_values = aod_values + bias_values
...
plt.title("AOD CMAQ")
plt.savefig('aod_cmaq_map.png')  # Save the figure
plt.show()
