...
def addmodelvar_derived(restartpath):
  ...
  print (delp.shape)
  print (pm2p5.shape)
  pm2p5_per_layer = scale_ug2kg*delp*pm2p5/9.8
  pm2p5_per_area = np.sum(pm2p5_per_layer,axis=0)
  pm2p5_column_total = 0.0*pm2p5
  ...
  print (nlev)
  ilev = 0
  pm2p5_per_area_tmp = pm2p5_per_layer[0,:,:]
  pm2p5_column_total[ilev,:,:]=pm2p5_per_layer[0,:,:]
  for ilev in range(1, nlev):
    pm2p5_per_area_tmp=pm2p5_per_area_tmp+pm2p5_per_layer[ilev,:,:]
    pm2p5_column_total[ilev,:,:]=pm2p5_per_area_tmp
    print(ilev,np.sum(pm2p5_per_layer[ilev,:,:]),np.sum(pm2p5_per_area_tmp))
    #print(np.sum(pm2p5_per_layer[ilev,:,:]))
    #print(np.sum(pm2p5_per_area_tmp[:,:]))
  var_derived = pm2p5_per_area
  print(var_derived.shape)

  # add z- and Time dimensions
  var_derived = np.array([[var_derived for i in range(u.dimensions['zaxis_1'].size)]],np.float32)
  print(var_derived.shape)
  var_derived[0,:,:,:]=pm2p5_column_total
