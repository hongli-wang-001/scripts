...
output_netcdf_file = 'filtered_std_6by6_output.nc'
variables = [
...
def stddev_filter(data, size=3):
    """
    Calculate the standard deviation in a neighborhood defined by size.
    """
    def stddev(input_array):
        return np.std(input_array, ddof=0)  # ddof=0 for population std deviation

    return generic_filter(data, stddev, size=size, mode='constant', cval=0.0)
...
#filtered_data = median_filter(data, size=(3, 3))
        ...
        for t in range(time_dim):
            for z in range(z_dim):
             #   filtered_data[t, z, :, :] = median_filter(data[t, z, :, :], size=(6, 6))
             filtered_data[t, z, :, :] =  stddev_filter(data[t, z, :, :], size=(3, 3))
