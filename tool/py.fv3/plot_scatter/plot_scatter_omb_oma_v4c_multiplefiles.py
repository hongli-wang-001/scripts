...
input_pattern = '../3dvar*.nc'  # Change to your file path and pattern
...
def plot_scatter(data, title, filename, color, label):
    ...
    x_min = np.min(data_flat)
    x_max = np.max(data_flat)

    # Set y-axis limits
    y_min = -1  # Minimal y value to ensure all points are visible
    y_max = 1    # Small positive value for visibility
...
for group in groups:
    ...
    label = group
    plot_scatter(combined_data, f'Combined Scatter Plot - {group}', None, color, label)
