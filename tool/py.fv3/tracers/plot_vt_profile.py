...
with nc.Dataset(netcdf_file, 'r') as ds:
    ...
    # Average over xaxis_1 and yaxis_1 dimensions
    averaged_data = np.mean(data_3d, axis=(1, 2))  # Shape: (zaxis_1)

# Plot the averaged data
z_dim = averaged_data.shape[0]
z = np.arange(z_dim)

plt.figure(figsize=(10, 6))
plt.plot(z, averaged_data, marker='o', linestyle='-', color='b')
plt.xlabel('Z Dimension')
plt.ylabel('Averaged Value')
plt.title('Averaged Data Over xaxis_1 and yaxis_1 Dimensions')
plt.grid(True)

# Save the plot to a file
plt.savefig('averaged_data_plot.png')  # Save the figure with the desired filename
print("Figure saved as 'averaged_data_plot.png'.")
