...
# Reverse the order of the z dimension
z_dim = mean_data.shape[0]
z = np.arange(z_dim)[::-1]  # Reversed z dimension
mean_data_reversed = mean_data[::-1]  # Reverse mean data
median_data_reversed = median_data[::-1]  # Reverse median data

# Plot the data with reversed z dimension
plt.figure(figsize=(12, 8))
plt.plot(mean_data_reversed, z, marker='o', linestyle='-', color='b', label='Average')
plt.plot(median_data_reversed, z, marker='x', linestyle='--', color='r', label='Median')
plt.ylabel('Z Dimension')
plt.xlabel('Value')
plt.title('Average and Median Data Over xaxis_1 and yaxis_1 Dimensions (Reversed Z Dimension)')
plt.legend()
plt.grid(True)

# Save the plot to a file
plt.savefig('average_median_data_plot_reversed_z.png')  # Save the figure with the desired filename
print("Figure saved as 'average_median_data_plot_reversed_z.png'.")
