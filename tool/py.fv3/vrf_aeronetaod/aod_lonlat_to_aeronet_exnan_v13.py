...
# Interpolation to observation locations
interpolated_aod = np.full_like(valid_obs_aod, np.nan)
if valid_obs_aod.size > 0:
...
aod_difference = interpolated_aod - valid_obs_aod
...
with nc.Dataset('aod_aeronetexcnon_cmaq_interpolated.nc', 'w', format='NETCDF4') as nc_out:
    nc_out.createDimension('points', len(valid_obs_aod))
...
plt.savefig('aod_cmaq_interpolated_excnan.png')
plt.show()
...
scatter = ax.scatter(longitude_lcc, latitude_lcc, c=valid_obs_aod, cmap='YlOrRd', edgecolor='k', alpha=0.7)
ax.add_feature(cfeature.COASTLINE, linestyle='--')
ax.add_feature(cfeature.BORDERS, linestyle=':')
ax.add_feature(cfeature.STATES, linestyle='-')
#ax.gridlines(draw_labels=True)
...
plt.savefig('aod_aeronet_excnan.png')
plt.show()
...
scatter = ax.scatter(longitude_lcc, latitude_lcc, c=aod_difference, cmap='RdBu_r', edgecolor='k', alpha=0.7)
ax.add_feature(cfeature.COASTLINE, linestyle='--')
ax.add_feature(cfeature.BORDERS, linestyle=':')
ax.add_feature(cfeature.STATES, linestyle='-')
#ax.gridlines(draw_labels=True)
...
plt.savefig('aod_difference_excnan.png')
plt.show()
