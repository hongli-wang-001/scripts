...
# Attention!!! Change (0,0) on the center 2d grid
    grid_xt = grid_xt - cen_x_adj
    grid_yt = grid_yt - cen_y_adj
    lon = np.zeros((ny, nx))
    lat = np.zeros((ny, nx))
    xt2d = np.zeros((ny, nx))
    yt2d = np.zeros((ny, nx))
...
valid_mask = (obs_x >= grid_xt.min()*dx) & (obs_x <= grid_xt.max()*dx) & \
             (obs_y >= grid_yt.min()*dy) & (obs_y <= grid_yt.max()*dy)    

#num_elements = len(obs_x)  # or len(obs_y), assuming both are the same length
#valid_mask = np.full(num_elements, True)  # Create an array of True values
