...
def extract_data(nc_file, group_name, var_name, missing_value):
    with nc.Dataset(nc_file, 'r') as dataset:
        ...
        print(f"Sample data from {group_name}:")
        print(data[:10])  # Print a few sample values for debugging
        data_masked = np.ma.masked_equal(data, missing_value)  # Mask missing values
        print(f"Sample masked data from {group_name}:")
        print(data_masked[:10])  # Print a few sample masked values for debugging
    return data_masked

# Function to print statistics
def print_statistics(data, group_name):
    data_flat = data.compressed()  # Get array without masked values
    print(f"Data (compressed) for {group_name}:")
    print(data_flat[:10])  # Print a few sample values for debugging

    if len(data_flat) == 0:  # Check if there are no valid data points
...
with nc.Dataset(output_file, 'w', format='NETCDF4') as new_nc:
    ...
    for i, group in enumerate(groups):
        ...
        min_val = max(np.min(combined_data), min_limit)  # Apply min limit
        max_val = min(np.max(combined_data), max_limit)  # Apply max limit
        ...
        missing_var[i] = missing_count  # Number of missing values
