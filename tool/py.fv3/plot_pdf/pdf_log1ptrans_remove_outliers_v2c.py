...
for i, group in enumerate(groups):
    ...
    # Plot histogram of log-transformed data (Original Data)
    counts, bins_log = np.histogram(transformed_data_dict[group].flatten(), bins=bins_log)
    bin_centers_log = (bins_log[:-1] + bins_log[1:]) / 2
    ax.bar(bin_centers_log, counts, width=np.diff(bins_log), alpha=0.7, edgecolor='black', label='Original Data')
    
    # Plot KDE fit for original data
    kde_original = gaussian_kde(all_log_values, bw_method='scott')
    pdf_original = kde_original(x)
    ax.plot(x, pdf_original * np.max(counts) / np.max(pdf_original), 'r-', lw=2, label='KDE Fit (Original)')
    ax.set_xlabel('Log(Value)')
    ax.set_ylabel('Frequency')
    ax.set_title(f'{group} (Log Transformed)')
    ax.legend()

    # Plot histogram of cleaned log-transformed data (without outliers)
    ax = axes[i + 1]
    counts_cleaned, bins_log_cleaned = np.histogram(cleaned_log_values, bins=bins_log)
    bin_centers_log_cleaned = (bins_log_cleaned[:-1] + bins_log_cleaned[1:]) / 2
    ax.bar(bin_centers_log_cleaned, counts_cleaned, width=np.diff(bins_log_cleaned), alpha=0.7, edgecolor='black', label='Cleaned Data')
    
    # Plot KDE fit for cleaned data
    ax.plot(x, pdf * np.max(counts_cleaned) / np.max(pdf), 'r-', lw=2, label='KDE Fit (Cleaned)')
    ax.set_xlabel('Log(Value)')
    ax.set_ylabel('Frequency')
