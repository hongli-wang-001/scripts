...
def print_statistics(data, group_name):
    ...
    min_val = max(np.min(data_flat), min_limit)  # Check min limit
    max_val = min(np.max(data_flat), max_limit)  # Check max limit
...
with nc.Dataset(output_file, 'w', format='NETCDF4') as new_nc:
    ...
    for i, group in enumerate(groups):
        ...
        missing_var[i] = missing_values_count[group]  # Number of missing values
