...
input_file = './3dvar_nicas_pm25.nc'  # Adjust this to your file path
...
# use same min and max in x and y in scatter figure, respectively
    x_min_fig = min(x_min, y_min)
    y_min_fig = x_min_fig
    x_max_fig = max(x_max, y_max)
    y_max_fig = x_max_fig
...
# Customize the color limits
    ...
    plt.title(title)
    plt.xlim(x_min_fig, x_max_fig)
    plt.ylim(y_min_fig, y_max_fig)
    plt.grid(True)
    # Draw the diagonal line
    plt.plot([x_min_fig, x_max_fig], [y_min_fig, y_max_fig], color='red', linestyle='--', linewidth=2, label='Diagonal')
    plt.text(0.25, 0.95, f'X Min: {x_min:.2f}\nX Max: {x_max:.2f}\nX Mean: {x_mean:.2f}\nX Std Dev: {x_std_dev:.2f}\n\n'
...
plot_scatter(x_data, y_data, "Scatter Plot of Oman vs Ombg PM2.5", "scatter_dens_plot.png")
