...
for hour in range(0, 24):
    ...
    file_list = sorted(glob.glob(f"vrf_6hcyc_pm_h100v5/*??/aod.*_{fhr}.nc"))
    if not file_list:
    ...
    for file in file_list:
        with nc.Dataset(file, 'r') as ds:
           ...
           # Load aod obs 
            if 'aod500nm' not in ds.variables:
                raise KeyError(f"'aod' not found in {file}")
            aod = ds.variables['aod500nm'][:]
    ...
    with nc.Dataset(out_file, 'w', format='NETCDF4') as ds_out:
        ...
        aod_mean_var = ds_out.createVariable('aod_obs__mean', 'f4', ('y', 'x'), zlib=True)
